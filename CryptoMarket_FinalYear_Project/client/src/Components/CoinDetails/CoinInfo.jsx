...
export default function CoinInfo({ state, open }) {
  ...
  useEffect(() => {
    if (currencyRupee === true) {
      setCoindata({
        current_price: ((`${data.current_price}` / 1) * 1).toLocaleString(
          "en-IN",
          {
        ...
        high: ((`${data.high_24h}` / 1) * 1).toLocaleString("en-IN", {
          maximumFractionDigits: 2,
          style: "currency",
          currency: "INR",
        }),
        low: ((`${data.low_24h}` / 1) * 1).toLocaleString("en-IN", {
          maximumFractionDigits: 2,
          style: "currency",
          currency: "INR",
        }),
        priceChange: ((`${data.price_change_24h}` / 100) * 83).toLocaleString(
          "en-IN",
          {
            maximumFractionDigits: 2,
            style: "currency",
